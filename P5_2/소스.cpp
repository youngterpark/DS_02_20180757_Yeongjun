...
// 고찰
// 중위식 표현법은 일반적으로 연산자가 피연산자 사이에 배치되는 표현식이다.
// 후위식 표현법은 연산자가 피연산자 뒤에 배치되는 표현식이다.
// 후위식 표현법은 계산이 빠르다. 컴퓨터에게 좋다.
// 중위식에서 후위식으로 변환할 때 연산자의 우선순위가 중요하다.
// 템플릿은 다양한 (int, char) 자료형을 사용할 수 있어서 편한 것 같다.
